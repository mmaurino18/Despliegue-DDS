...
public class CambiarContraseña {
    public boolean puede_cambiar(String contra_nueva, String contra_actual, ArrayList<String> contra_antiguas) {
        ...
        if(validacion.validar(contra_nueva) && !peores_contras.estaContenida(contra_nueva) && !this.sonIguales(contra_nueva,contra_actual) && !this.esUnaContraAntigua(contra_nueva,contra_antiguas)) {
            return true;
        } else {
            ...
            this.errores(contra_nueva,contra_actual,contra_antiguas);
            return false;
        }
    }
    ...
    private void errores(String contra_nueva, String contra_actual, ArrayList<String> contra_antiguas){
        if(this.sonIguales(contra_nueva,contra_actual)){
            System.out.println("* Las contraseña nueva es igual a la actual.Tienen que ser diferentes");
        }
        if(this.esUnaContraAntigua(contra_nueva,contra_antiguas)){
            System.out.println("* Las contraseña nueva es igual a una contraseña anterior.No se puede repetir");
        }
    }
}